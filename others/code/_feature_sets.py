# Collection of functions to extract the different sets of features
...
# function to extract basic abs, basic rel, readability and lexical diversity features

# ru_words_pattern = re.compile(r'[а-яА-Я]+')
...
def get_basic(doc,raw,stop_words,google10000):
 ...
 # basic abs and basic rel
	...
	# word-list
	wordlist = [str(x).lower() for x in re.findall(r'[a-zA-Z]+',raw)] 
	# word-length-list
	lenlist = np.array([len(x) for x in re.findall(r'[a-zA-Z]+',raw)])
	# punctuation-list
	punctuation_ = re.findall(r'[.,:;?!-\"\(\)\[\]\n]',raw)
	# sentence-length-list (same word-definition as for wordlist)
	sentlist_ = []
	for sent in doc.sents:
		sentlist_.append(len([x for x in re.findall(r'[a-zA-Z]+',str(sent))]))
	...
	# number of characters (letters in words, punctuation)
	chars = np.sum(lenlist) + len(punctuation_)
	# number of difficult words (>3 syllables)
	dwords = textstat.difficult_words(raw)
	# number of short words (<4 characters)
	swords = np.sum(lenlist < 4)
	# number of long words (>6 characters)
	lwords = np.sum(lenlist > 6)
	# number of stopwords (from spacy 'stop_words'-list)
	stopwords_ = 0
	...
	basic_abs = np.array([chars,sylls,words,sentences,dwords,swords,lwords])
	
	if words == 0:
		return basic_abs, np.zeros(6), [-21.43,-15.8,0,206.835,0,0,0,0,3.1291], np.zeros(5), words, sentences, punctuation_, wordlist
		
	basic_rel = np.array([chars/words,sylls/words,words/sentences,dwords/words,swords/words,lwords/words])
...
# lexical diversity
	...
	lexical_div = np.array([stopwords_/words,uniquewords_/words,top100,top1000,top10000])
...
def get_formatting(raw,punctuation_,dict_,sentences,words):
	...
	# probability distribution over punctuation
	punctuation_dist = np.array(list(dict_.values())) / np.sum(list(dict_.values()))

	# punctuation-tokens per sentence
	punctuation_sent = np.array(list(dict_.values())) / sentences
	
	# PARAGRAPHS
	iter = re.finditer(r'\n+',raw)
	breaks = [b.start(0) for b in iter]
	
	if not breaks:
		return punctuation_dist, punctuation_sent, np.array([1,words])
	
	paragraphs_ = []
	breaks.insert(0,0)
	for i in range(len(breaks)-1):
		paragraph_ = raw[breaks[i]:breaks[i+1]]
		paragraphs_.append(len([x for x in re.findall(r'[a-zA-Z]+',paragraph_)]))
		
	# number of paragraphs 
	num_paragraphs = np.sum(np.array(paragraphs_)>0)
	# average length of paragraphs
	avglen_paragraphs = np.mean([x for x in paragraphs_ if x>0])
...
def get_rep(doc):
	...
	for sent in doc.sents:
		...
		wordlist = [str(x).lower() for x in re.findall(r'[a-zA-Z]+',sent.string)] # consistent definition of word
		...
		if len(wordlist) != 0:
			overlap_word.append(overlap_word_/len(wordlist)) # share of words that have already been in prev. sentence

		if len(poslist) != 0:
			overlap_pos.append(overlap_pos_/len(poslist)) # share of words that have already been in prev. sentence
	...
	overlap_hist_word_ = np.histogram(overlap_word[1:],bins=np.arange(0,1.1,0.1))[0] 
	overlap_hist_word = overlap_hist_word_ / np.sum(overlap_hist_word_)
	overlap_hist_pos_ = np.histogram(overlap_pos[1:],bins=np.arange(0,1.1,0.1))[0] 
	overlap_hist_pos = overlap_hist_pos_ / np.sum(overlap_hist_pos_)
	
	overlap_hist_word_bi_ = np.histogram(overlap_word_bi[1:],bins=np.arange(0,1.1,0.1))[0] 
	overlap_hist_word_bi = overlap_hist_word_bi_ / np.sum(overlap_hist_word_bi_)
	overlap_hist_pos_bi_ = np.histogram(overlap_pos_bi[1:],bins=np.arange(0,1.1,0.1))[0] 
	overlap_hist_pos_bi = overlap_hist_pos_bi_ / np.sum(overlap_hist_pos_bi_)

	overlap_hist_word_tri_ = np.histogram(overlap_word_tri[1:],bins=np.arange(0,1.1,0.1))[0]
	overlap_hist_word_tri = overlap_hist_word_tri_ / np.sum(overlap_hist_word_tri_)
	overlap_hist_pos_tri_ = np.histogram(overlap_pos_tri[1:],bins=np.arange(0,1.1,0.1))[0]
	overlap_hist_pos_tri = overlap_hist_pos_tri_ / np.sum(overlap_hist_pos_tri_)
	
	lexical_rep = np.concatenate((overlap_hist_word,overlap_hist_word_bi,overlap_hist_word_tri))
	syntactic_rep = np.concatenate((overlap_hist_pos,overlap_hist_pos_bi,overlap_hist_pos_tri))
				...
				if not str(doc[token.i-1]) == '': # make sure direct context is non-empty
					if not doc[token.i-1].is_digit: # make sure direct context is no number
						if not str(doc[token.i-1]) == '\n':
							if not str(doc[token.i-1]) in ['more','around','ever','on','fewer','over','lots','so']: # make sure direct context is no established figure-of-speech
...
def get_syntactic(doc,dict_,words,sentences):
	...
	dist_lite_ = np.array(list(dict_.values())[:5])
	...
	type_per_types = dist_lite_ / np.sum(dist_lite_)
	unique_type_per_types = dist_lite_unique_ / np.sum(dist_lite_unique_)
	type_per_sent = dist_lite_ / sentences
	unique_type_per_sent = dist_lite_unique_ / sentences
	
	pos_features = np.concatenate((type_per_doc,unique_type_per_doc,type_per_types,unique_type_per_types,type_per_sent,unique_type_per_sent))
...
def get_ne(doc,dict_,words,sentences):
	...
	if (len(doc.ents) == 0) or (words == 0):
		return np.zeros(18), np.zeros(18), np.zeros(5), [], 0
	...
	for cluster in clusters:
		...
		for i in range(len(cluster.mentions)):
			...
			if i < (len(cluster.mentions)-1):
				inference_distance = cluster.mentions[i+1].start - cluster.mentions[i].start
				total_inferences += 1
...
def get_grid(doc,dict_):
	...
	for sent in doc.sents:
		...
		wordlist = [str(x).lower() for x in re.findall(r'[a-zA-Z]+',str(sent))]
		if wordlist:
	...
	entities = [entity for entity in entities if len(entity[1]) == len(entity[2])]
	ents = len(entities)
...
def get_redundancy(doc,wordlist,lemma=False):
	...
	if lemma == False:
		...
		for sent in doc.sents:
			...
			wordlist_sent = [str(x).lower() for x in re.findall(r'[a-zA-Z]+',str(sent))]
		...
		wordlist = []
		for token in doc:
			str_lemma = re.findall(r'[a-zA-Z]+',str(token.lemma_))
			if not str_lemma:
				continue
	...
	redundancy_features = np.array([information_loss,A_mean,A_median,A_max,A_min,A_mean_diff,A_median_diff,A_max_diff,A_min_diff])
...
def get_empath(empath_,words):
	...
	# empath features
	empath_features = np.array([empath_score,empath_mean,empath_median,empath_min,empath_max,empath_var,empath_act])
